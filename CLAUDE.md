...
- ⚙ **Claims about the app's network access appear in five places** — `docs/index.html` (feature card),
  `docs/llms.txt`, `docs/datenschutz.html`, `docs/privacy.html`, and AI_MASTER. The app makes exactly two calls:
  exchange rates (`api.frankfurter.dev`) and the manual update check (`api.github.com`). Keep all five in
  agreement.
